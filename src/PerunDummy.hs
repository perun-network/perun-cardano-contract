...
import Data.Data
import Data.Map as Map hiding (drop, keys, map)
...
extractVerifiedState (SignedState sigs) signingKeys =
  let states =
   ...
   in if all (== head states) (tail states)
        then
          ( case head states of
              Just s -> s
...
-- | This returns `Just s`, iff the signature on the channel state `s`
-- | is valid under the given public key and `Nothing` otherwise.
